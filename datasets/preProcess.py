...
# Normalizar a coluna 'y' manualmente entre 0 e 1
y_min = data['y'].min()
y_max = data['y'].max()
data['y'] = (data['y'] - y_min) / (y_max - y_min)

# Verificar se a normalização está correta
print(f"Valor mínimo de 'y' após normalização: {data['y'].min()}")
print(f"Valor máximo de 'y' após normalização: {data['y'].max()}")
...
# Adicionar coluna de timestamps simulados (a cada segundo)
